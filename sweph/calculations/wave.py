...
def total_wave(ordered, pos_map):
...
def store_wave(data):
    ...
    use_varga_cycle = data["use varga cycle"]
...
def calculate_wave(event: str):
    ...
    for dt in price_df.index:
        ...
        wave = total_wave(members_ordered, pos_map)
        wave_vals.append(wave)
    wave_df = pd.DataFrame({"datetime": price_df.index, "cycle": wave_vals})
    # print(f"cycledf : {cycle_df}")
    wave_data = {
        "members": members,
        "division": division,
        "use varga cycle": use_varga_cycle,
        "dataframe": wave_df,
    }
    store_wave(wave_data)
    msg += f"wavedata :\n{wave_data}\n"
    app.signal_manager._emit("wave_changed", event, wave_data)
    notify.debug(
        msg,
        source="wave",
        route=["terminal"],
