...
from sweph.constants import TERMS, NAKSATRAS27, MANSIONS28
...
# from ui.mainpanes.chart.ringsouter import (
#     Transit,
#     TransitVarga,
#     P2Progress,
#     P3Progress,
...
#     LunarReturn,
#     Harmonic,
#     Naksatras, Signs, Event, Info
# )


def draw_triangle(cr, x, y, size, angle_rad):
    cr.save()
    cr.translate(x, y)
    cr.rotate(angle_rad)
    cr.move_to(0, -size)
    cr.line_to(size * 0.866, size * 0.5)
    cr.line_to(-size * 0.866, size * 0.5)
    cr.close_path()
    cr.fill()
    cr.restore()
...
class BaseRing:
    def __init__(self, radius, cx, cy, font_size, radius_dict=None):
        self.radius = radius
        self.cx = cx
        self.cy = cy
    ...
    def lon_to_xy(self, lon, r):
        angle = math.pi - math.radians(lon)
        x = self.cx + r * math.cos(angle)
        y = self.cy + r * math.sin(angle)
        return x, y


class Signs(BaseRing):
    def __init__(self, radius, cx, cy, font_size, stars=None, radius_dict=None):
        super().__init__(radius, cx, cy, font_size, radius_dict)
        self.stars = stars or {}
    ...
    def draw(self, cr):
        ...
        cr.stroke()
        cr.arc(self.cx, self.cy, inner_r, 0, 2 * math.pi)
        cr.stroke()

        mid_r = (self.radius + inner_r) / 2.0
        sign_glyphs = list(glyphs.SIGNS.values())
        for i in range(12):
            lon = i * 30.0
            x1, y1 = self.lon_to_xy(lon, inner_r)
            x2, y2 = self.lon_to_xy(lon, self.radius)
            cr.move_to(x1, y1)
            cr.line_to(x2, y2)
            cr.stroke()
            sign_tuple = sign_glyphs[i]
            glyph = (
                sign_tuple[0] if isinstance(sign_tuple, (list, tuple)) else sign_tuple
            )
            glyph_x, glyph_y = self.lon_to_xy(lon + 15.0, mid_r)
            cr.select_font_face()


class Rings:
    """manages drawing chart rings & collecting snap targets"""

    def __init__(self, ctx: dict, events_data: dict):
        self.ctx = ctx
        ...
        self.outer_rings = ctx.get("outer_rings", [])
        self.extra_info = ctx.get("extra_info", {})
        self.chart_settings = ctx.get("chart_settings", {})
        self.notify = ctx.get("notify")
        self.movie_mode = ctx.get("movie_mode", False)
        self.snap_targets = []

    def draw(self, cr):
        e1_data = self.events_data.get("e1", {})
        e2_data = self.events_data.get("e2", {})
        houses = e1_data.get("houses", ())
        ascmc = houses[1] if houses and len(houses) >= 2 else None
        fixed_asc = self.chart_settings.get("fixed_asc", False) and ascmc
        if fixed_asc and ascmc:
            asc_angle = math.radians(ascmc[0])
            cr.save()
            cr.translate(self.cx, self.cy)
            cr.rotate(asc_angle)
            cr.translate(-self.cx, -self.cy)
        font_size = int(12 * self.font_scale)
        map_rings_outer = {  #
            "transit": (
                Transit,
                {
                    "transit_data": e2_data.get("transit"),
                    "stations": e2_data.get("stations"),
                },
            ),
            "transit varga": (
                TransitVarga,
                {"transit_varga_data": e2_data.get("transit_varga")},
            ),
            "p2 progress": (
                P2Progress,
                {
                    "p2_pos": e2_data.get("p2_pos"),
                    "stations": e2_data.get("p2_stations"),
                },
            ),
            "p3 progress": (
                P3Progress,
                {
                    "p3_pos": e2_data.get("p3_pos"),
                    "stations": e2_data.get("p3_stations"),
                },
            ),
            "p3m progress": (
                P3MinorProgress,
                {
                    "p3m_pos": e2_data.get("p3m_pos"),
                    "stations": e2_data.get("p3m_stations"),
                },
            ),
            "d1 direction": (
                D1PrimaryDirection,
                {
                    "d1_pos": e2_data.get("d1_pos"),
            ...
            "lunar return": (
                LunarReturn,
                {
                    "lun_ret_data": e2_data.get("lun_ret_data"),
                },
            ),
            "solar return": (
                SolarReturn,
                {
                    "sol_ret_data": e2_data.get("sol_ret_data"),
                },
            ),
            "harmonic": (
                Harmonic,
                {
                    "notify": self.notify,
                    "division": int(
                        self.chart_settings.get("harmonic ring", "0").strip() or 0
                    ),
                    "harmonic_data": e1_data.get("harmonic_data"),
                },
            ),
            "naksatras": (
                Naksatras,
                {
                    "naks_num": (
                        28 if self.chart_settings.get("28 naksatras", False) else 27
                    ),
                    "first_nak": int(self.chart_settings.get("1st naksatra", 1)),
                },
            ),
        }
        for ring_key in self.outer_rings:
            if ring_key in map_rings_outer:
                cls, kwargs = map_rings_outer[ring_key]
                radius = self.radius_dict.get(ring_key, self.max_radius)
                ring_inst = cls(
                    radius=radius,
                    cx=self.cx,
                    cy=self.cy,
                ...
                ring_inst.draw(cr)
                if hasattr(ring_inst, "snap_targets"):
                    self.snap_targets.extend(ring_inst.snap_targets)
        signs_r = self.radius_dict.get("signs", 0.0)
        ring_signs = Signs(
            radius=signs_r,
            cx=self.cx,
            cy=self.cy,
            font_size=int(signs_r * 0.07),
            stars=e1_data.get("stars", {}),
            radius_dict=self.radius_dict,
        )
        ring_signs.draw(cr)
        if hasattr(ring_signs, "snap_targets"):
            self.snap_targets.extend(ring_signs.snap_targets)
        event_r = self.radius_dict.get("event", 0.0)
        cusps = houses[0] if houses and len(houses) >= 1 else []
        ring_event = Event(
            radius=event_r,
            cx=self.cx,
            cy=self.cy,
            font_size=int(event_r * 0.08),
            guests=e1_data.get("positions", {}),
            cusps=cusps,
            ascmc=ascmc or [],
            chart_settings=self.chart_settings,
            stations=e1_data.get("stations"),
            lots=e1_data.get("lots"),
            eclipses=e1_data.get("eclipses"),
            syzygy=e1_data.get("syzygy"),
            radius_dict=self.radius_dict,
        )
        ring_event.draw(cr)
        if hasattr(ring_event, "snap_targets"):
            self.snap_targets.extend(ring_event.snap_targets)
        # restore rotated
        if fixed_asc:
            cr.restore()
        info_r = self.radius_dict.get("info", 0.0)
        ring_info = Info(
            self.notify,
            radius=info_r,
            cx=self.cx,
            cy=self.cy,
            font_size=int(info_r * 0.17),
            chart_settings=self.chart_settings,
            event_data=e1_data.get("chart_info", e1_data),
            extra_info=self.extra_info,
            use_mean_node=self.chart_settings.get("mean node", False),
            movie_info=e1_data.get("positions", {}),  # ???
