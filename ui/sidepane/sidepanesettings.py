...
class SidepaneSettings(CollapsePanel):
    ...
    def build_subpnl_objects(self) -> CollapsePanel:
        ...
        sel_objs = (
            ...
            if self.sidepane.selected_objects_event == "e1"
    ...
    def build_subpnl_chartsettings(self) -> CollapsePanel:
        ...
        for setting in ["mean node", "exact lunar month"]:
            row = Gtk.ListBoxRow()
            tooltip = chart_settings.get(setting, (False, ""))[1]
        ...
        ent_info.connect(
            "activate", help.chart_info_string, "chart info", self.sidepane
        )
        ...
        # ent_info_extra.set_width_chars(20)
        # ent_info_extra.set_max_width_chars(20)
        ent_info_extra.set_text(str(self.app.dispatcher.chart_info_extra))
        ent_info_extra.connect(
            "activate", help.chart_info_string, "chart info extra", self.sidepane
        )
    ...
    def build_subpnl_ayanamsa(self) -> CollapsePanel:
        ...
        ayan_store = Gtk.StringList.new([
            f"{val[1]}"  # if isinstance(val, (tuple, list)) else str(val)
            for val in ayanamsas
        ])
    ...
    def build_subpnl_files(self) -> CollapsePanel:
        ...
        for row, (key, value) in enumerate(files.items()):
            ...
            ent_files.connect("activate", help.files_changed, key, self.sidepane)
